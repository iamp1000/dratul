...
def create_new_patient(
    ...
    crud.create_activity_log(db, user_id=current_user.id, action="Create Patient", details=f"Created new patient: {new_patient.name} (ID: {new_patient.id})")
    return new_patient

@router.get("/", response_model=List[schemas.Patient])
def read_all_patients(skip: int = 0, limit: int = 200, db: Session = Depends(get_db)):
    patients = crud.get_patients(db, skip=skip, limit=limit)
    return patients

@router.get("/{patient_id}", response_model=schemas.Patient)
def read_patient_details(patient_id: int, db: Session = Depends(get_db)):
    db_patient = crud.get_patient_details(db, patient_id=patient_id)
...
@router.post("/{patient_id}/remarks/", response_model=schemas.Remark)
